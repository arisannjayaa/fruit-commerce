					...
					<div class="card-body">
						<div class="row">
							...
							<div class="col-6 text-end">
								<p class="h3" id="fullname">{{ $customerDetail->first_name . ' ' . $customerDetail->last_name ?? ''}}</p>
								<address>
									<div id="email">{{ $customerDetail->email }}</div>
									<div id="telephone">{{ $customerDetail->phone }}</div>
									<div>
										<span id="address">{{ $customerDetail->shipping_address->address }}</span> <br>
										<span id="state">{{ $customerDetail->shipping_address->country_code == "IDN" ? "Indonesia" : "" }}</span>,
										<span id="city">{{ $customerDetail->shipping_address->city }}</span> <br>
										<span id="postal-code">{{ $customerDetail->shipping_address->postal_code }}</span>
									</div>
							</div>
							<div class="col-12 my-5">
								<h1>Invoice {{ $transaction->order_id }}</h1>
								<p>Tanggal Pembelian : {{ formatDateId($transaction->created_at) }}</p>
							</div>
						</div>
